...
W4 第一条竖切片已实现：Codex 与 Claude Code 的结构化事件归一为 session/result/tool/message，ExecutionReceipt 保存请求模型、平台回报模型、会话、token 和费用（平台缺失时保持 unknown）。工作台提供用户显式触发的有界连接验证作业，固定最小提示、隔离临时目录、输出上限、超时和同 Agent 并发保护；结果持久化但不保存登录凭证或原始验证事件。验证成功只把 authentication 置为 ready，evaluationSupport 仍为 exploratory。自动化使用 fake CLI 覆盖收据、结构化错误和验证持久化；真实账号的一致性矩阵尚未执行。
...
- `tests/` 覆盖领域、工作台、发布和 CLI 端到端回归。mock 与 fake CLI 只用于工程验证，不能作为平台或模型效果结论。
