...
状态：实施中的目标架构，更新于 2026-09-17；W1–W3 主要竖切片与 W4 的统一平台收据、连接验证、双平台事件语义、能力诊断矩阵和用户触发的一致性验证作业已实现。真实账号矩阵尚未由用户执行，完整 SQLite 发布/导出索引仍按本文推进。本文基于当前 v0.1 代码及用户的六项需求：本地启动网页、发现并复用本机 Agent、启动目录作为工作区、执行过程可视化、结果保存对比、Skill 版本管理。
...
首版使用 React + TypeScript + Vite 构建前端，Node 本地服务承载 API、调度和存储。开发入口 `npm run dev` 与 `skillbenchmark ui` 已实现，支持 `--workspace`、`--port`、`--no-open`；当前已接通工作区、Agent 探测、Skill/Suite/计划，以及 Run 启动、增量事件查询、结果持久化、取消、结果详情和两次已完成运行的比较。计划可冻结试跑、有效性对照或同一 Skill 的 v1/v2 版本对照；版本对照分别绑定并物化 incumbent/candidate，报告展示版本增益、回归和门禁。计划冻结不会调用 Agent，只有显式启动 Run 才执行；当前详情复用完整报告和有界事件窗口，对比复用两份持久化报告并将 Run ID 保存到 URL。游标补流仍按后续切片实现。
...
当前“验证连接”会创建可持久化的有界作业：固定最小提示、隔离临时目录、60 秒上限、256 KiB 输出上限并禁止同一 Agent 并发验证。它仅在用户点击后调用模型，页面提示可能产生少量费用。成功后 authentication 可标记 ready，但 evaluation_support 仍为 exploratory；只有完整能力矩阵通过后才可升级为 verified。

Agent 卡片的诊断矩阵按能力逐项显示状态、证据来源和解释。平台未报告字段使用 `not-reported`；自动化夹具验证使用 `adapter-conformance`，并明确说明它不代表当前本机 CLI 已实测。这样可避免把“代码路径有测试”“本机安装可启动”和“真实账号下能力已验证”混成一个绿色徽标。

完整一致性验证必须由用户选择模型并确认可能费用后启动。它依次执行收据、受限只读工具、硬超时和取消探针，逐项持久化进度并允许用户取消；同一 Agent 不并发验证。只有全部必需项以及实际模型、会话、用量和费用都有证据时才升级 `evaluation_support=verified`，否则保留 exploratory 和缺失原因。
...
下列为 `/api/v1` 目标操作；工作区、资产、计划、运行、发布以及连接验证主链路已提供，其余仍按表内范围渐进实现：

| 操作 | 路由示例 | 行为 |
| --- | --- | --- |
| 工作区与 Agent | `GET /workspace`、`GET /agents`、`POST /agents/refresh` | 获取当前上下文及重新探测 |
| Agent 验证 | `POST /agents/:id/verify`、`POST /agents/:id/consistency-verifications`、`GET /agent-verifications/:id`、`POST /agent-verifications/:id/cancel` | 创建有界连接/一致性作业，查询逐项进度并取消活动作业 |
...
W1–W4 合起来覆盖用户本轮六项需求。W2 先形成可用的单平台路径，W3/W4 补齐历史比较、版本与双平台体验。W4 当前已完成统一收据、连接验证、双平台事件夹具、能力诊断矩阵和完整一致性验证作业；真实账号矩阵尚未由用户主动执行。真实 Agent 测试应采用明确的小型 Suite 和预算，工程测试使用可控 fake CLI 覆盖异常路径；两类证据分别记录。
