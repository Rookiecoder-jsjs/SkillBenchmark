...
首版使用 React + TypeScript + Vite 构建前端，Node 本地服务承载 API、调度和存储。开发入口 `npm run dev` 与 `skillbenchmark ui` 已实现，支持 `--workspace`、`--port`、`--no-open`；当前接通工作区和 Agent 探测，资产、运行与历史 API 仍按后续切片实现。
...
交付界面示例数据必须标记 demo。W1 第一条竖切片已改变程序行为并通过自动化与浏览器验收；后续仍按 W1–W4 的任务和协议迁移推进，不把尚未接通的页面入口描述为可用能力。
