...
状态：实施中的目标架构，2026-09-16；工作台、Agent 探测、Skill/Suite 不可变资产、评测计划冻结，以及 W2 的首个 Adapter 运行/事件/取消切片已实现。真实账号一致性验证、完整结果对比与发布体验仍按本文推进。本文基于当前 v0.1 代码及用户的六项需求：本地启动网页、发现并复用本机 Agent、启动目录作为工作区、执行过程可视化、结果保存对比、Skill 版本管理。
...
首版使用 React + TypeScript + Vite 构建前端，Node 本地服务承载 API、调度和存储。开发入口 `npm run dev` 与 `skillbenchmark ui` 已实现，支持 `--workspace`、`--port`、`--no-open`；当前已接通工作区、Agent 探测、Skill/Suite/计划，以及 Run 启动、增量事件查询、结果持久化和取消 API。计划冻结不会调用 Agent，只有显式启动 Run 才执行；详细结果比较仍按后续切片实现。
...
模型“跟随本机默认”是便捷选项；计划记录请求的配置，执行收据记录平台报告的实际模型、配置和版本。无法确定时标记 unknown，缺失身份的历史运行不能认定为严格可比。复用账号与继承所有背景 Skill/记忆不是同一件事，继承范围纳入执行 Profile；以能力检查选择隔离选项，不强加会破坏本机认证的统一 flags。
...
交付界面示例数据必须标记 demo。W1 的工作台/Agent 探测、Skill 资产、Suite 资产与评测计划，以及 W2 的 Adapter 运行监控切片已改变程序行为并通过自动化与浏览器验收；fake CLI 结果只证明工程链路，真实平台效果仍须单独验证。
