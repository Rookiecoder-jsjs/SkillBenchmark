...
状态：实施中的目标架构，更新于 2026-09-17；W1–W3 主要竖切片与 W4 的统一平台收据、显式连接验证、双平台事件语义和能力诊断矩阵已实现。真实账号完整一致性矩阵和完整 SQLite 发布/导出索引仍按本文推进。本文基于当前 v0.1 代码及用户的六项需求：本地启动网页、发现并复用本机 Agent、启动目录作为工作区、执行过程可视化、结果保存对比、Skill 版本管理。
...
| 验证连接 | `POST /agents/:id/verify`、`GET /agent-verifications/:id` | 创建有界验证作业并查询持久化终态 |
...
W1–W4 合起来覆盖用户本轮六项需求。W2 先形成可用的单平台路径，W3/W4 补齐历史比较、版本与双平台体验。W4 当前已完成统一收据、连接验证、双平台事件夹具和能力诊断矩阵；完整真实账号一致性矩阵仍未完成。真实 Agent 测试应采用明确的小型 Suite 和预算，工程测试使用可控 fake CLI 覆盖异常路径；两类证据分别记录。
