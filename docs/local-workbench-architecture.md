...
状态：下一阶段设计提案，2026-09-16；本文描述目标行为，不代表网页、服务和新协议已经实现。基于当前 v0.1 代码及用户的六项需求：本地启动网页、发现并复用本机 Agent、启动目录作为工作区、执行过程可视化、结果保存对比、Skill 版本管理。
...
首版使用 React + TypeScript + Vite 构建前端，Node 本地服务承载 API、调度和存储。开发入口 `npm run dev` 与 `skillbenchmark ui` 已实现，支持 `--workspace`、`--port`、`--no-open`；当前已接通工作区、Agent 探测和 Skill 目录导入/版本查询，测试集、运行与历史 API 仍按后续切片实现。
...
交付界面示例数据必须标记 demo。W1 的工作台/Agent 探测和 Skill 资产两条竖切片已改变程序行为并通过自动化与浏览器验收；后续仍按 W1–W4 的任务和协议迁移推进，不把尚未接通的页面入口描述为可用能力。
