...
状态：实施中的目标架构，2026-09-16；工作台、Agent 探测、Skill/Suite 不可变资产和评测计划冻结已实现，真实执行、实时过程、结果对比与发布体验仍按本文推进。本文基于当前 v0.1 代码及用户的六项需求：本地启动网页、发现并复用本机 Agent、启动目录作为工作区、执行过程可视化、结果保存对比、Skill 版本管理。
...
首版使用 React + TypeScript + Vite 构建前端，Node 本地服务承载 API、调度和存储。开发入口 `npm run dev` 与 `skillbenchmark ui` 已实现，支持 `--workspace`、`--port`、`--no-open`；当前已接通工作区、Agent 探测、Skill 目录导入/版本查询、Suite 版本冻结和评测计划预览。计划冻结不会调用 Agent；运行、实时事件与历史比较 API 仍按后续切片实现。
...
交付界面示例数据必须标记 demo。W1 的工作台/Agent 探测、Skill 资产、Suite 资产与评测计划三条竖切片已改变程序行为并通过自动化与浏览器验收；后续仍按 W1–W4 的任务和协议迁移推进，不把尚未接通的真实运行入口描述为可用能力。
