...
export function gradeTrial(spec: TrialSpec, task: Task, receipt: ExecutionReceipt, artifact: Artifact | null): Grade {
  if (receipt.status === "errored") return { schema_version: "0.1", trial_id: spec.trial_id, grader_digest: GRADER_DIGEST, outcome: "ungradable", metrics: { exact_match: 0 }, assertions: [{ name: "execution_completed", passed: false, detail: receipt.failure_reason ?? "infrastructure error" }], evidence_refs: [] };
