...
本仓库处于架构设计阶段。先通过 README.md 阅读设计文档，再按 docs/implementation-plan.md 的 P0 验收条件逐步引入实现。TypeScript、SQLite 等目前是拟定选型。
...
依赖管理器和运行时在实现阶段选定后记录于 README.md，提交对应锁文件。当前无需安装依赖，尚无构建、测试或发布命令。
